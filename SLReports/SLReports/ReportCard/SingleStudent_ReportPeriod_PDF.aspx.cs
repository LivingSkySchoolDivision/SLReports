...
{
    public partial class SingleStudent_ReportPeriod_PDF : System.Web.UI.Page
    {
        ...
        protected PdfPTable classWithMarks(SchoolClass course, PdfContentByte content)
        {
            ...
            foreach (ReportPeriod reportPeriod in course.ReportPeriods)
            {
                foreach (ObjectiveMark objectivem in course.ObjectiveMarks)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(objectivem.cMark))
                        {
        ...
        protected void sendPDF(List<MemoryStream> PDFData)
        {
            Response.Clear();
            Response.ClearContent();
            Response.ClearHeaders();
            ...
            foreach (MemoryStream memstream in PDFData)
            {
